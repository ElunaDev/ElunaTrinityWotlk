...
{
    if (!sWorld->getBoolConfig(CONFIG_BATTLEGROUND_CROSSFACTION) || bg->isArena())
...
bool BattlegroundQueue::FillXPlayersToBG(BattlegroundBracketId bracket_id, Battleground* bg, bool start)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_CROSSFACTION) && (sBattlegroundMgr->isTesting() || queuedPeople >= bg->GetMinPlayersPerTeam() * 2 || !start))
    {
